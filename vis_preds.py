...
if __name__ == '__main__':
    ...
    if not path_contact_states:
        # Use the annotated contact states
    ...
    for i, item_name in enumerate(item_names):
        ...
        for k in range(item_len):
            ...
            for n, joint_ids in enumerate(contact_ids):
                ...
                for j, j_ctt in enumerate(joint_ids): # j_ctt: joint id in contact_states
                    ...
                    if not len(ctt_states[j_ctt]) == nclasses+1:
                        raise ValueError(
                            "check failed: len(ctt_states[j_ctt]) == nclasses+1 ({0} vs {1})".format(len(ctt_states[j_ctt]), nclasses+1))
                    if not np.sum(ctt_states[j_ctt][:-1]) == 1:
                        raise ValueError(
                            "check failed: np.sum(ctt_states[j_ctt][:-1]) == 1")
            ...
            # (required by ffmpeg if we want to convert image sequences to videos)
            w_target = w/2*2
            h_target = h/2*2
